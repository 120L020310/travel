# Travel Reverie 7.8.1 · Editable Cover & Context Menu
...
- 右键任一地点相框可打开专属照片库：支持一次添加多张、勾选单张或多张删除，以及为单张照片上移／下移调整轮播顺序；项目默认图片固定保留。
- 默认照片也可替换为本地图片，并可恢复为项目原图；任一自定义照片可设为首图。右键相框仅显示小型“编辑照片库”菜单，点击其他位置会自动关闭该菜单。
